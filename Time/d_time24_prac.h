...
{
   public:
      time24(int h = 0, int m = 0, string p = "am");
...
time24::time24(int h, int m, string p) : hour(h), minute(m), period(p)
{
...
void time24::addTime(int m)
{
   ...
   int oldhours, newhours, checkperiod;
   
   newhours = (m + minute)/60;
   minute = (m + minute) % 60;
   
   checkperiod = (hour + newhours)/12;
   
   
   if (hour != 12)
   {	
   	if (period == "am" && checkperiod % 2 != 0)
   	{
   		period = "pm";
   	}
   	else if (period == "pm" && checkperiod % 2 != 0)
   	{
   		period = "am";
   	}
   }
   ...
   /*if (12 - checkperiod <= 0)
   {	
   	cout << checkperiod << endl;
   	checkperiod = checkperiod/12;
   	cout << checkperiod << endl;
   ...
   extrahours = (m + minute)/60;
   extrahours = hour + extrahours;
   
   minute = (m + minute) % 60;
   
   hourdiff = 12 - extrahours;
   
   if (hourdiff <= 0)
   {
   	hourdiff = hourdiff / 12;
   
   	if (period == "am" && hourdiff % 2 != 0)
   	{
   		period = "pm";
   	}
   	else if (period == "pm" && hourdiff % 2 != 0)
   	{
   		period = "am";
   	}
   }
...
{
   // output time in format of hour: minute period
